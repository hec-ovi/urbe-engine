...
- `src/server/`: development routes; `/api/building` validates and builds a missing preview, `/api/launcher` owns catalog and creation calls, `/api/talk` connects NPC dialog to the configured text model, and `/api/speech` validates local synthesis and transcription traffic.
...
- `src/quest-bundle/`: atomic consumer and selector for the Quests v0.8.0 handoff v1.1: definitions, objectives, investigations, fixed mechanic bindings, mission asset requests, item bindings and host capabilities (src/quest-bundle/CONTRACT.md).
...
- `src/game/`: the playable city (`?mode=game`); first-person controller on Rapier, night scene from the assembled GLBs, ground from the blueprint's cover polygons, neon and lit windows, simulation-driven crowd and lane-graph traffic, doors into continuous interiors (src/game/CONTRACT.md). Depends on ../atlas, ../connections, ../materials, ../simulation contracts.
  ...
  - `voice/`: GameApp dialogue lifecycle, verified Chatterbox Nano synthesis, faster-whisper microphone transcription, absolute PCM playback and request-scoped local process or HTTP service cancellation (src/game/voice/CONTRACT.md)
