...
- Correction on our own reference: https://github.com/depixeled-chris/gta7 uses **no physics engine**. Its dependencies are `lucide`, `simplex-noise` and `three@^0.160.0`. Collisions and driving are hand-rolled velocity-vector arcade physics. Take it as a reference for feel and for its architecture (simulation core kept three.js-free and testable in node, rendering in a separate layer), not as a physics integration reference.
...
- Textures: KTX2 / Basis (`KHR_texture_basisu`). UASTC for normal and ORM maps, ETC1S for albedo and facades. Encode with `ktx create` / `ktx encode` from KTX-Software v4.4.2 (current stable; v5.0.0 is still at rc2, do not adopt yet). The legacy `toktx` CLI is deprecated and slated for removal in 4.5. Loader: `KTX2Loader` with `setTranscoderPath(...)` (or `setTranscoderUrls(...)`) plus `detectSupport(renderer)` so it transcodes to the device's native BC/ETC2/ASTC format. Textures stay GPU-compressed after transcode; PNG and JPG expand to full RGBA in VRAM.
...
- Use `SkyMesh` from `three/addons/objects/SkyMesh.js`. It is the WebGPU/TSL sibling of the classic `Sky`, built on `NodeMaterial`, and it is the correct one for our renderer. `Sky` is the WebGL-only version; same model, same parameters.
...
- Fog: `FogExp2(color, density)` with colour and density driven off sun elevation. There is no built-in height fog; write a TSL one only if the flat exponential visibly fails against tall towers.
