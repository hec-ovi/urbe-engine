...
| How many distinct buildings a city may share | A 1 km city stands 378 kit buildings on 273 plans. Atlas hands each parcel its own floor band (105 bands over 431 parcels), so half the lots of a template slot cannot stand the slot's count and mint a plan at their own bound, and each plan also carries its parcel's type and tier. With both inputs as they are, the best possible sharing is 222 plans; one programme per slot reaches 145; letting a lot stand under its Atlas minimum (never over its cap) reaches 64. A quarter of the buildings needs a ruling on at least one of the two | Engine, Atlas, Exterior |
| Prepared street/link/station/fixture/model assets with collision and streaming bounds; shared dimensions catalog | Ordinary streets and kit buildings draw from published piece kits; Engine still constructs link, station, fixture and highway geometry from current inputs and imports producer constants. Producer ownership and shared dimensions require an agreed consuming schema | Engine, Atlas, Streets, Connections/Links, Exterior, Interior, Materials, Modeling |
