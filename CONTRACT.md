...
- launcher request: [src/server/schema/launcher-request.schema.json](src/server/schema/launcher-request.schema.json), POST `/api/launcher` for catalog, continue, JSON import/export, staged creation and revisioned game saves; browser request and response bodies use [src/launcher/schema/launcher-api.schema.json](src/launcher/schema/launcher-api.schema.json), while stored cities and games use the exact schemas in [src/library/CONTRACT.md](src/library/CONTRACT.md)
