...
`?mode=game` takes its whole run from the URL: `world` (atlas sample), `out` (assembled directory), `backend` (`webgpu` default, `webgl`), `hour` (world clock, default 21), `crowd` and `cars` (instance capacity), `density` (street population scale, default 1). Debug views are off unless asked for: `lanes=debug` paints the whole lane graph.
...
- **Neon** is hung by the game: blade signs and ad screens on the facade each parcel's street access faces, lit window panes behind a deterministic share of the windows, thin emissive strips along the lane centerlines, lamp posts every 19 m on alternating kerbs and one on each junction corner. A fixed pool of point lights follows the nearest signs and lamps, so real light on the road costs the same in a village and in a city.
