...
- **Buildings** merge across the city by material key, so the skyline costs one draw call per key. Shells come from each parcel's own exterior GLB, under a megabyte each; the furnished interiors are tens of megabytes and stream separately, one building at a time, nearest first, only while you are within 70 m of it. What arrives is cut into rooms and into floor bands, and only the bands within one floor of where you stand are in the scene and solid, so walking up the stairs moves the window. Interiors stay at their real world position in the same scene, so entering a building means walking through its door with no load and no camera jump. Ground floor doors hang their leaf on a hinged pivot and swing on E.
