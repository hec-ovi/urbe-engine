...
describe( 'a companion under way', () => {
	...
	it( 'calls out while waiting for a lagging player, at most every three quarters of a minute', () => {
		...
		for ( let second = 0; second < 150; second ++ ) {
			...
			const timeMin = AFTERNOON + second / 60;
		...
		for ( let index = 1; index < called.length; index ++ ) expect( called[ index ] - called[ index - 1 ] ).toBe( 45 );
	...
	it( 'gives up on a player who walks away and says so', () => {

		const game = setup();
		const mira = game.start( 'follow' );
		const far = [ 5000, 1, 5000 ];
		let signals = [];
		for ( let timeMin = AFTERNOON; timeMin <= AFTERNOON + 3.5 && ! signals.length; timeMin += 0.25 ) signals = game.frame( timeMin, far );
...
function setup( quests = {}, crowd = null, restored = null ) {
	...
	const game = {
		...
		/** A request made standing beside the person, in the place they are. */
		ask( { npcId, position, place }, timeMin = AFTERNOON ) {

			return { npcId, timeMin, playerPosition: [ position[ 0 ] + 1, position[ 1 ], position[ 2 ] ], playerPlaces: place?.kind === 'parcel' ? [ place ] : [] };
