...
- A place is named by its own name, else by its venue word through `name-unnamed` (`the clinic`), `name-stop` or `name-station`. Offered places that share a name carry `offeredAs`: the name with the compass point they lie toward from the person, through `name-north` to `name-north-west` (`the clinic to the north`, north being -z), and for two that lie the same way also the walk to 10 m through `name-away` (`the clinic to the north, 240 m away`). A place that reads the same as a better one in name, point and walk is passed over for the next, except a quest place. The offer label, the accept line and the talk offers use `offeredAs`; the saved destination, the arrival lines and the `guide` use the plain `name`. Labels, accept and refusal lines come from `lines.md` by key; the variant is fixed by the person and the minute.
