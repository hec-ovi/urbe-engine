...
- **Buildings**: every assembled merged GLB. Shells merge across the whole city by material key, so the skyline costs one draw call per key. Each building's interior stays a group at its real world position **in the same scene**: entering is walking through a door, never a load or a swap. Inside that group the interior is cut into its published rooms, and a room is drawn only while it is within reach and on the level the player is on. Interiors are hidden and their colliders dropped beyond a radius measured on the ground plane, so a tower's upper floors count as near their own building.
