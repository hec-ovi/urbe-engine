...
- Per parcel the manifest names, under `<out>/<id>/`: the exterior blueprint and shell GLB, and `interior/building.glb`, `interior/floors/*.json` plus `interior/npc.json`. A blueprint parcel the manifest does not name is reported as unbuilt, never an error.
...
- **Interiors, streamed**: a furnished building is tens of megabytes and up to sixty floors, and the player stands on one of them, so neither the file nor the whole tower is ever fully in play. The interior GLB is fetched only while its building is within 70 m of the player, nearest first, one at a time, and dropped past 95 m. What comes back is cut into the published rooms and into floor bands, and only the bands within one floor of where the player stands are in the scene and in the physics world: walking up the stairs moves the window. The material a mesh wears is its key plus the variant the interior box asked for (`extras.materialVariant`), so a patterned ceiling and a plain one stay two looks of one entry. A room is drawn only while it is within reach and on the level the player is on. Interiors stay at their real world position **in the same scene**: entering is walking through a door, never a load screen or a swap.
...
- **Sky**: `SkyMesh` with the sun below the horizon, carrying the skyglow a lit city throws back off its air so no window is a hole cut out of the frame; a moon key at the illuminance a full moon really gives; stars.
