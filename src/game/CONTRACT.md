...
- [Building sources](data/schema/world-buildings.d.ts): Exterior blueprint for every listed parcel, with a GLB for a shell parcel and a placement table for a kit one; NPC data and per-floor JSON/GLB only for manifest interiors. Source reads share eight concurrent requests. Catalogs above 250 shells select nearby sources plus all interiors; smaller or legacy worlds load every listed source.
