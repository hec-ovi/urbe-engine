...
- [World manifest](../assembly/schema/world-manifest.schema.json): exact shell IDs, selected interiors, floor tags, source versions and optional archive, Connections, shell catalog, rooftop, piece kit and per-parcel source references. The carried blueprint is preferred. `kit` names the copied `kit/kit.json` and its sha256; `sources` maps a parcel id to `kit` or `shell`, and a parcel marked `kit` requires that kit. `interiorModules` names the copied `interior-modules/modules.json` and its sha256, and a world with interiors must publish it; the optional `interiorProps` names the copied furniture catalog the same way. A manifest without them is a world of generated shells. A declared Connections artifact requires matching seeds and byte hashes; manifests without it retain in-process Connections generation and sample fallback.
...
- `WorldSource.load()` returns the [building-source ports](data/schema/world-buildings.d.ts), Atlas and Connections data, optional quest catalogs, NPC types, the requested game descriptor and `kit` (the validated `kit.json` and the base URL its piece files are relative to, or null). `interiorModules` and `interiorProps` are the shared interior catalogs with the base URL their files are relative to, or null. `nativeStreets` is the [verified saved street source](ground/native/CONTRACT.md), or null for legacy worlds. A declared street source requires its exact carried blueprint and bundle hashes; missing data fails loading. `loadBuildings(ids)` reads unique manifest IDs without owning residency. `unbuilt` lists Atlas parcels absent from the manifest.

Game composes native ordinary streets with retained ground, highways and stations through `GroundScene`. Native features reserve prop clearance independently of residency. Exact replacement owners suppress legacy hardware; ordinary native paint is emitted once while highway paint remains in its existing builder. Rendering and physical collision consume the same original street geometry.
...
Ground and props use local rendering windows with collision within 256 m. Source dimensions, transforms, material keys and placement plans remain unchanged. Original shells stream for large catalogs; selected interiors remain available to Simulation. A furnished floor is its layout's module and furniture placements at that floor's elevation, drawn from catalogs loaded once for the city, and waits for complete collision before it enters those draws.
...
- `E_KIT_PIECES` and `E_KIT_PLACEMENT`: an unreadable piece kit or a placement table naming a piece it lacks, published by the [Kit runtime](city/kit/CONTRACT.md).
