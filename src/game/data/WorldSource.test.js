...
const kitDocument = {
	version: 1, exterior: '0.0.0', seed: 'plans',
	plans: [ {
		id: 'tower', family: 'white-grid', baysAcross: 1, baysDeep: 1, floors: 1,
		glb: 'plans/0123456789abcdef/tower.glb',
		blueprint: 'plans/0123456789abcdef/tower.blueprint.json',
...
describe( 'WorldSource selective interiors', () => {
	...
	it( 'composes a kit parcel blueprint from its plan, reading that plan once for the city', async () => {
		...
		// Its own building id, and every point of the plan in the world frame.
		expect( p0.buildingId ).toBe( 'p0' );
		expect( floor.kind ).toBe( 'lobby' );
		// A quarter turn about the point the plan's origin stands at.
		expect( floor.outline ).toEqual( [ [ 100, 200 ], [ 100, 192 ], [ 108, 192 ], [ 108, 200 ] ] );
		expect( floor.openings[ 0 ].edge ).toBe( 0 );
		expect( world.buildings.get( 'p1' ).blueprint.floors[ 0 ].outline[ 0 ] ).toEqual( [ 300, 400 ] );
