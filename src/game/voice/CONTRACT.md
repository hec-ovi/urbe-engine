...
- `new NpcVoice({ dialog, types, persona, hold, enabled, volume, client, player, cache })` takes the type definitions (`types`), `persona(npcId)` and `hold(conversation, seconds)` directly, plus the Voice client, Web Audio player and session cache.
...
- `silenced()` aborts the download, stops the audio at once and drops every queued line and waiting batch. When a batch went out since the last silence, it also cancels the session's prefetch group through `DELETE /api/voice/prefetch/<group>`, sent once that batch's request is answered so Voice sees them in order. Turning voice off does the same.
...
- `GET /api/voice` is asked before the first line. While it is not `ok`, after a line fails with 503, or once two lines in a row fail with 502 (Voice answers but its model server does not; `status` reads `degraded`), no line is voiced for 30 s, then it is asked again. Any failure is logged and leaves the conversation as it was.
