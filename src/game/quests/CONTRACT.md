...
- Every active goto or talk step whose place is a parcel gets an area ring at that parcel's anchor, across every questline, so a side job's venue is marked while the main story is being followed. A talk step also stands its cast at that parcel through `Crowd.castMember` and puts one mark over each of their heads, only while the runtime has that step open: a body posted into a step the runtime would refuse is a conversation that does nothing. While the step is closed the ring stays, nobody is stood there, and the HUD and the quest log say why.
