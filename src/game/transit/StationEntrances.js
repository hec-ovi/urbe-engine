...
/**
 * Every station's entrances, straight off the atlas: at each entrance point a
 * stair descends under the pavement between two walls, the walls carry on
 * above the pavement as a balustrade around the opening, and a portal at the
 * mouth holds a lit band naming the mode. The descent runs towards the station
 * itself, which is the one direction the blueprint fixes for it.
 *
...
export class StationEntrances {
		...
		for ( const mode of MODES ) {
			...
			for ( const station of this.atlas.transit?.[ mode.collection ] ?? [] ) {
				...
				for ( const entrance of station.entrances ) {
					...
					const matrix = placement( entrance, station.position );

					if ( ! matrix ) continue;

					structure.push( model.structure.clone().applyMatrix4( matrix ) );
	 ...
	 * running the way the stair descends. Every concrete part is solid: the
	 * player walks down the treads and along the landing, and cannot step over
	 * the balustrade into the shaft.
	 */
	#model() {
		...
		// The shaft is closed at the far end: past here is the station, which
		// this box does not build.
		...
		for ( const sx of [ - 1, 1 ] ) {
			...
			const wall = box( WALL, top - floor, length );
			wall.translate( sx * ( CLEAR + WALL ) / 2, ( floor + top ) / 2, length / 2 - 0.15 );
			parts.push( wall );

			const post = box( WALL + 0.06, PORTAL_TOP - top, WALL + 0.06 );
			post.translate( sx * ( CLEAR + WALL ) / 2, ( top + PORTAL_TOP ) / 2, - 0.02 );
			parts.push( post );
