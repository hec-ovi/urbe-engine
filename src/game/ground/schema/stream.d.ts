...
/** One fixed body of world cuboids under an id, solid the moment it is admitted. */
export interface CuboidCollisionPort {
	addBoxes(id: string, boxes: { center: [number, number, number]; halfExtents: [number, number, number]; rotationY: number }[]): boolean;
}
export interface GroundWindow {
	...
	/** Native streets admit cuboid bodies; authored ground admits triangle bands. */
	collision?: BandCollisionPort & CuboidCollisionPort;
...
export interface GroundStream {
	...
	/** `indexed` counts the source records the stream owns, tiles or placements. */
