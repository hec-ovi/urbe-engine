...
- `GroundScene(atlas, factory, nativeSource?, textureOptions?, {catalog?, buildings?}?)`: returns the Ground stream surface for the whole ground scene. A [native source](native/CONTRACT.md) selects [native residency](native-stream/CONTRACT.md) plus only its declared retained Atlas ground. Highways and station structures remain authored by their existing builders. Native textures use [source options](materials/schema/ports.d.ts); original source resolution is retained unless a preparation callback budgets it. The complete original Atlas remains available to every planning consumer. Disposal releases the owned streams, native materials and native textures; the input source remains caller-owned.
...
- GroundStream returns the [stream surface](schema/stream.d.ts), including its group, global bounds and residency counters. Tiles own each complete source cover and every module repetition once, with their exact source boundaries. Visible geometry uses ordinary indexed material meshes in 512 m pages on every renderer backend. Exact source positions, normals and metre UVs expand in 4 ms cooperative slices. Unchanged pages retain their buffers; replacements prepare before publication. Preparation receives each changed page group. Collision expands only nearby render triangles into arrays of at most 2,048 triangles for Physics admission. Eviction releases page buffers and tile geometry; disposal releases cached templates. No whole-city collision buffer is allocated. Initial updates may omit ports and attach them later.
- With a native source, saved streets draw as one `BatchedMesh` per native surface over a city-wide piece kit and collide as one cuboid body per placement cell. Retained Atlas ground, highways and station structures keep their own builders and their triangle collision.
...
- Nonmodule ground covers use their authored `top`; curb faces use their `bottom`. Where standing building sources are supplied, cover and fitted-paving triangles stop at the shell catalog's actual world-space ground-band outlines, falling back to loaded blueprints' ground-floor outlines for older worlds. The cut respects the building's elevation, arbitrary rotation, concave outlines and footprints crossing several ground owners; it leaves empty lots and paving outside the built footprint intact. Render attributes interpolate across each cut, and collision uses the same cut boundaries. Original Atlas records remain unchanged. Native street pieces and highway structures retain their own published geometry.
- Fitted paving expands only the supplied integer spans and snapped frame corners into bodies and four disjoint joint strips. Shared edge stations remain vertices through rendering outside standing-building cuts. No cell is inferred or backed by another face. Solid parts retain their exact polygon and role. Collision triangulates the owner polygons and exposed curb faces before the same building cuts.
