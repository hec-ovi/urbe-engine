...
Each GLB must contain the declared triangle count, surfaces, collision tags and native vertex attributes. Bounds must match the source within Float32 precision. Material resources finish loading before admission. `prepare` runs before publication and when its port changes. Missing assets, material failures and invalid geometry reject without a fallback.
