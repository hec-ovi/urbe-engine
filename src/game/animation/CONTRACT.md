...
The live game adapter synchronizes NPC continuity routines, turns accepted quest actions into commands, and sends every ordered transition to Crowd plus the one focused HeroCharacter armature. The focused person's crowd body keeps the clip it shows until the armature has taken its slot (or the load was dropped) and takes the transition's terminal clip then, unless a later transition gave it one. So the armature starts from the pose that body shows and plays an entry whole. Coordinator state is presentation derived from persisted quest and NPC continuity state, so it is not duplicated in the game save.
NPC reply gestures hold for 1.8 seconds unless the player sends another line or closes the conversation first. The adapter's `speaking(conversation, speech)` passes whose voice plays, and when it ends, to the focused armature (`HeroCharacter.speak`, [agents](../agents/CONTRACT.md)), whose head and neck move to it over whatever clip plays.
