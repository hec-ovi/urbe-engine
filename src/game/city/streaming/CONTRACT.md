...
The optional loader port follows `BuildingsLoader.load(sources)`. A cell is admitted with its group hidden and turns visible once the rebuilt skyline no longer carries it, so a port whose draws live outside the cell group waits for that flag before it draws anything. [KitCellLoader](../kit/CONTRACT.md) is that port for a kit world: it reads the building plans its parcels stand on, the first cell in the city to need one paying for it, appends kit parcels as copies into city-owned material batches when the stream shows that cell, hands landmark parcels to the original GLB loader, and adds `boxColliders` for the cell. A plan it cannot read leaves those parcels as empty lots and the cell still stands. `ShellScene` gives that whole cell to `WorldColliders.addBoxes` as one fixed body and releases it by cell id on eviction.
