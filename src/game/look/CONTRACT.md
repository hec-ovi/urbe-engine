...
- `Warmup(renderer, scene, camera, mrt, renderTarget?)`: `warm(object)` waits for its maps to decode and uploads each new map once before compiling its pipelines. Each upload yields a frame, or a short timer where the browser has stopped drawing frames, so preparation never waits on a frame that is not coming. `warmAll(object, {wanted?, onProgress?})` prepares one renderable per program, a material with the vertex layout it is drawn with, one at a time with a frame yield between them; the optional boolean callback cancels before the next one. Progress receives completed and total programs. A program this warm-up already built, in any pass, is never built again, so the city pass prepares only what the ground and the props left. Both return elapsed milliseconds. Compilation uses the visible world pass's render target, MRT and linear colour state, restoring them afterward. Hidden and frustum-culled objects, including instance and material batches standing empty, are staged only during compilation and restored exactly; inactive lights stay inactive.
