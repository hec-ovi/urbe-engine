...
- `NightFog(scene, { density, color, indoorDensity = 0 })`: installs height fog outdoors and clear air in enclosed rooms. `update(air, indoor, delta)` fades the street medium out on entry and restores it on exit over 0.6 s, without changing shader identity or tinting the doorway with room light. An explicitly requested indoor medium carries 0.1 cd/m2 per lux of its fixtures' mean illuminance; the outdoor medium carries the sky's radiance.
