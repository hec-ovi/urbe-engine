...
- `importStory(input)`: [schema/import-story.schema.json](schema/import-story.schema.json). `recording` is a Quests recording directory holding `recording.json`.
...
- City creation requires only size. Omitted names and seeds receive fresh identities; explicit names and seeds remain supported. A plan's id is its city's id, and neither a plan nor a city may already hold it.
...
- `importStory` plays a recording an author wrote outside the engine against the draft's world and NPC types. Its questId is `<cityId>-story-<side jobs>`.
