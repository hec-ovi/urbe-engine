...
- Automatic interiors open the locations the ten-step main quest needs first, then side-job venues, then other venues, up to the requested count; a building Interior cannot furnish is skipped for the next candidate and `count` reports what opened. Fewer than seven opened is `E_QUEST_LOCATIONS`. Seven interiors support the main line, eight support two side jobs, and nine support all three.
