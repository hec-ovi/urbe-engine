...
- `loadGame`: [schema/game-descriptor.schema.json](schema/game-descriptor.schema.json). Its optional `transitJourney` preserves an exact waiting or aboard timetable state. Its optional `questTransit` preserves the active quest ride stage and identities. Its optional `npcState` is [schema/npc-state.schema.json](schema/npc-state.schema.json), preserving simulation replay, NPC materialization, interruption, companion and walk-home routes, an optional active quest escort, the optional [companion](../game/companion/CONTRACT.md) state and world time. Its optional `investigations` array preserves validated evidence discovery, collection and emitted transition state. Its optional `scenery` is the [saved scenery](../game/scenery/schema/saved-scenery.schema.json) lifecycle of every quest scene, and its optional `dialogueMemory` is [schema/dialogue-memory.schema.json](schema/dialogue-memory.schema.json): per person, the folded notes and recent turns Quests' dialog memory serializes.
...
- Simulation save schema, game NPC agents continuity-save schema, playable quest transit-state schema, companion state schema and scenery saved-state schema, gathered once in [src/DescriptorSchemas.js](src/DescriptorSchemas.js) for every boundary that validates a descriptor or a save. City resources and quest bundles cross this boundary as checksummed file-reference envelopes.
