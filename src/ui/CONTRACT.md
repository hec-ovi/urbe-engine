...
- **ChatPanel**: props `onSend(text)`, `onClose()`, `onChoice(value)`, `onTopic(value)`, `onAction(id)`, `onRetry()`, `onJournal()`. `show({ name, role? } | null)` opens a fresh conversation or closes the panel. `setNpc`, `setStory`, `setTopics`, `setChoices`, `setActions([{ id, label }])` (each button carries its id as `data-action`), `setStatus(text, { error?, retry? })`, `setSending`, `setFreeChat(available, note?)`, `setTranscript` and `setVisible` expose presentation only.
  - Lines: `addMessage({ from, name, text })` appends a whole line and returns its element. `beginMessage({ from, name })` opens a line that grows as it arrives and returns `{ line, update(text), finish(), discard() }`, where `update` shows the text so far; `discard()` removes an unfinished line, and calls on a line that finished, was discarded or left the transcript do nothing. The log is `aria-busy` while a line grows. `setSpeaking(line, 'pending' | 'playing' | 'idle')` marks how a shown line is voiced, returns false for a line not in the transcript and throws on another state. A growing line ends in a caret and a voiced line carries a marker; both blink only when the player has no reduced-motion preference.
...
- **MissionSummary**: the story card. Props `onClose({ pointer })`, `onOpen()`; `show({ kind: 'outcome' | 'prologue', title, text, outcome: 'done' | 'failed', steps: [{ text, done }] })`, `setVisible( bool )`; it opens as a named modal focused on its action, retains focus after text clicks, and the action or close button reports `{ pointer: true }`, Escape `{ pointer: false }`. `outcome` (the default, and any kind the layout lacks) shows the outcome badge and continue; `prologue` shows the Prologue line and Begin. A blank line in `text` starts a paragraph. Labels come from [summary-layout.json](widgets/summary-layout.json), [schema](widgets/summary-layout.schema.json). GameView hides it before forwarding `onSummaryClose(close)`; the host releases pointer lock on open and takes it back on a click close, while Escape leaves the pointer free where the game stood.
...
## Components
`dom.el`, `Icon.icon( name )`, `KeyCap.keyCap( text )`, `PanelHeader` (title, key hint, Esc button), `EmptyState.emptyState( text )`, `Prose.prose( text )` (story text as paragraphs for a `prose` container), `SettingField`, `Button`, `SelectField`, `MenuButton.menuButton`. Stylesheets: `game.css` (palette and HUD chrome), `tabbar.css`, `panels.css`, `views.css`, `chat.css`, `mission.css`, `launcher.css` (front door and creation stages), `styles.css` (viewer and experiment overlays).
