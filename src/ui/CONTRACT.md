...
- **MissionSummary**: the story card. Props `onClose()`, `onOpen()`; `show({ kind: 'outcome' | 'prologue', title, text, outcome: 'done' | 'failed', steps: [{ text, done }] })`, `setVisible( bool )`; it opens as a named modal focused on its action, retains focus after text clicks, and the action or Escape reports the close. `outcome` (the default) shows the outcome badge and continue; `prologue` shows the Prologue line and Begin. A blank line in `text` starts a paragraph. Labels come from [summary-layout.json](widgets/summary-layout.json). GameView hides it before forwarding `onSummaryClose`; the host releases pointer lock on open and restores gameplay control on close.
