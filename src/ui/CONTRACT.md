...
- **ChatPanel**: props `onSend(text)`, `onClose()`, `onChoice(value)`, `onTopic(value)`, `onRetry()`, `onJournal()`. `setNpc`, `setStory`, `setTopics`, `setChoices`, `setStatus`, `setSending`, `addMessage`, `setTranscript`, `show(conversation|null)` and `setVisible` expose presentation only. The panel shows name/occupation, current story, a scrolling transcript, explicit reply buttons, optional free chat, journal and End conversation. Weekly schedules, parcel IDs and simulation state are not conversation text. Header, replies and exit stay reachable at narrow sizes. The modal retains keyboard focus when controls are replaced, supports Enter/Space replies and Escape close, and follows the newest message when its viewport resizes unless the player is reading earlier text. Pending free chat disables its composer only; authored story choices remain usable. Failures show a persistent explanation and Retry rather than an NPC ellipsis.
