...
- `POST /api/creation-jobs`: a [launcher request](schema/launcher-request.schema.json) for a creation stage: `planCity`, `buildCity`, `generateCity`, `generateInstances`, `generateQuests`, `importStory` or `createGame`.
- `GET /api/creation-jobs/<id>`: job id returned by POST.
- `POST /api/talk/stream`: [schema/talk-request.schema.json](schema/talk-request.schema.json). `line` holds 1 to 2000 characters, as the chat input does. `npc` is the Simulation NPCInstance the browser shows, including its optional `age`, `traits` and `transitJob`. `quests` is the optional exact `QuestSession.snapshot()`. Optional `offers` (`follow`, `places`) is what this NPC may propose, answered only on the stream; optional `guide` is the place it has led the player to, with notes of at most 2000 characters each. Optional `prior` is at most 12 lines `{ speaker: 'player' | 'npc', text, atMin }` of up to 4000 characters, oldest first: what the conversation showed since this person's last reply that no reply carried (a story opening, a choice and its reply, a greeting), each with the game minute it was said. The NPC's context carries them before `line`, and a completed exchange is remembered after them, each at its minute.
...
- The stages the browser front door does not call answer as their siblings do: `planCity` with `{ plan }`, the [plan](../creation/schema/plan-result.schema.json); `buildCity` with `{ city, catalog }` as `generateCity`; `importStory` with `{ quests }` as `generateQuests`.
- Creation job POST: HTTP 202 [job](schema/creation-job.schema.json) `{ id, method, state, submittedAt, startedAt, finishedAt, progress, result, error }`, queued; reading it returns HTTP 200 with the same shape. `progress` is the last line the stage's commands printed (Atlas, assembly, Quests), or null before the first. `state` is `queued`, `running`, `succeeded` with `result` the launcher result of that method, or `failed` with `error` `{ code, message }` as the launcher route would answer it.
...
- Creation job POST refuses a method other than the creation stages or an input its stage schema refuses with 400 `E_INVALID_REQUEST`, answers 503 `E_CREATION_UNAVAILABLE` without creation and 429 `E_BUSY` while four jobs wait or run; a job id this session does not keep is 404 `E_JOB_NOT_FOUND`. A stage that fails later stays visible in its failed job.
...
`TalkClient` ([../game/talk/TalkClient.js](../game/talk/TalkClient.js)) posts the request for `GameApp` to `/api/talk/stream`. `stream(conversation, line, timeMin, quests?, { signal?, guide?, offers? })` is an async iterator over the stream events up to `done`; leaving it early or aborting `signal` ends the reply on the server. `said(npcId, speaker, text, atMin)` notes a line the conversation showed at game minute `atMin` that no reply carried; the next `stream` to that person sends the newest 12 as `prior`, each cut to 4000 characters, and drops them once `done` arrives, so a failed reply keeps them. A line still waiting to go, such as an opening shown again when the talk reopens, is noted once. A line said with someone else starts the list again. `memory()` reads the world's dialogue memory and `restoreMemory(memory)` replaces it. Until the server has taken that memory, `stream` and `memory` hand it over first, one hand-over at a time, and fail when it cannot be taken, so no exchange is remembered without it and no late hand-over replaces one. Failures throw an `Error` whose `status` is the HTTP status, 502 for an `error` event or a stream that ends before `done`.
