...
- `want(family, bays, floors, use)` registers one and returns its record; `use` is the `{ type, tier }` the building is dressed for. The id is `<family or plain>-<type>-<tier>-<across>x<deep>x<floors>f`, with the programme left out when it is the family's own (`corpo` and `high_rich` for corporate sectors, `residential` and `high_rich` for the other five, `residential` and `mid` for a plain plan), so a name resolves to exactly one request and two programmes never share one plan's bytes.
...
- A plan with a family is generated with that family as `options.architecture`; a plan without one is generated with no architecture named, which is the generator's ordinary output on a plain rectangular plate. It is drawn for the programme of the parcels standing on it, their Atlas type and tier, so a hotel is dressed as a hotel and a rich street is never finished as a mid one, and a venue a passer-by reads off the street carries its word on the marquee (`HOTEL`, `MARKET`, `DINER`, `COFFEE`, `CLINIC`, `POLICE`). One building stands on many lots, so the sign it is drawn with is the venue word and never a parcel's own name, which stays in the placement record.
...
- A block Atlas tiled from a template is dressed by the template, not by its parcels. A stable hash of the world seed, the template id and the lot slot picks the family and the floor count for that slot, so every block of one template wears one design in each of its lots. A slot's family has to suit every parcel standing in it, so a mixed block never puts a luxury facade on a mid street; when its parcels share no family each of them picks its own, so a rich lot in a mixed slot still wears one. The slot's floor count stands inside every one of its parcels' envelopes; when those envelopes have no count in common, it is the count the most of them allow, so one plan stands on every block the slot repeats on. A lot whose own envelope excludes that count stands the nearest count it does allow and nothing else changes, wearing the slot's design at its own height. Each building is held inside the envelopes of the lots it covers, so a tower lot stands as a tower and none overruns its allowance.
- One variation per block instance: a stable hash of the block id either moves one slot's floor count by one within its fits, or merges two adjacent slots of equal depth into one rectangular building, whose lot stands one long building where the neighbouring blocks stand two. A merge is offered only for two lots one building suits: their envelopes have to leave a floor count in common and their uses a family. The merged-over lot ships nothing.
