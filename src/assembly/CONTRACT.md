...
Library `streets` defaults to false for stored-world publication compatibility; true uses the published native Materials binding, a seed derived by FNV-1a of `<atlas seed>:streets`, and wear1. [Street options](schema/street-options.d.ts) may supply seed, wear or a native binding object/path. Normal city generation always enables native streets. Archive plus native publication fails before staging/publication; the city CLI rejects archive input before producer work. Existing manifests without streets remain readable. JSON blueprint0.22.0/0.23.0 requires native reservations1.0.0, with no version downgrade.
