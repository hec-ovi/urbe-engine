...
0.33.0: creation asks no model: planCity plans a city, an author names it with Naming's CLI, buildCity builds it from that naming, generateInstances opens a home among the interiors of a named city, and importStory brings in a story an author wrote through Quests (external author mode), all as creation jobs. Worlds clone by hard links and keep unchanged documents linked, blueprints are written compact, and `npm run gc` sweeps unused shared sets with a one-day grace. Same-name places are told apart by their way, the view turns to a leader who arrives, staged bodies block pedestrians, saved dialogue memory reaches the server before any talk, request bodies are capped per route, and blood and tyre decals draw with their real coverage.

0.32.0: people follow the player or lead them to a place (their work, haunts, home, a quest place or a staged scene) when asked in chat or from the conversation's action row; a leader keeps pace, waits, gives up when left behind, walks through building doors and across floors, and on arrival the conversation opens and they talk about what is there. Quest scenery stands from quest specs while its conditions hold: bodies posed in the exact look of the person who died, blood and evidence, saved with the game, and investigations appear only with their step. What people remember of talking with the player is saved with the game. A speaking person's head and neck follow their voice, and the focused rig starts from the crowd's pose. Rooms stand without local-only furniture a machine lacks. Creation names themed cities and stages a story's scenes before play.

0.31.0: a person keeps their body, outfit, hair and eyebrows from the street through a conversation and after it: talk establishes the simulation identity in the look the crowd draws, handles only go to free bodies of the same gender, stress copies and retiring bodies talk anonymously, and the focused character wears the crowd variant, surface and hair tint. Free chat streams sentence by sentence, any player turn overtakes a reply still streaming, and errors say whether a retry can help. NPC lines are voiced by the Voice box (Maya1): text shows at once, audio streams per sentence through Web Audio, one person speaks at a time, authored choice replies render ahead, and voice turns itself off when the service is missing. Emotion cues in replies are voiced and hidden from the text.
...
0.28.0: paired building previews use the game’s walking, doors, lifts, streaming and adjustable inspection exposure. Source fingerprints rebuild stale pairs and publish them together; shared module identity includes actual model bytes, and garbage collection retains standalone previews. Interior floors can select additional layouts for tapering landmarks. Stair guardrails collide, rotated cores cut the shell floor correctly, and doorway thresholds and UV repeats agree with their modules.

Seated people use the actual furniture’s heading and cushion location, retaining their pose in conversations. Crowd and focused characters receive their room’s fixtures and bounce while retaining outdoor lighting. Native ground cover is cut beneath standing building footprints, so street paving no longer hides the interior floor. Quest appointments keep distinct people at their authored venues, preserve save progress when repairing old casts, gather listening partners together and support returning cast members. Authored character names agree across objectives, interaction prompts and conversations while simulation identities stay unchanged. Merged buildings use their standing footprint for location and interior objective guidance. Partial review worlds start at an open building; landmark selection respects cable attachment planes. Reusable review scripts build the four-copy and seven-family worlds and refresh their interiors without replacing their shells.
